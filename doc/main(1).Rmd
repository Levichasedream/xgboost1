...
if(run.feature.train){
  tm_feature_train <- system.time(dat_train_all <- feature(train_LR_dir, train_HR_dir))
  feat_train <- dat_train_all$feature
  label_train <- dat_train_all$label
}

save(dat_train_all, file="../output/feature_train.RData")
```
...
### Summarizing reproducibility on training model
...
## Implementation Using XGBoost Package

**Run this part after running line 1 - 106**
...
# source train and test functions
...
model_best_xgb=xgdepth[1]
if(run.cv){
  model_best_xgb <- xgdepth[which.min(err_cv_xgb[,1])]
}

par_best_xgb <- list(depth=model_best_xgb)
```

```{r}
# train with the cv selected param
tm_train_xgb=NA
tm_train_xgb <- system.time(fit_train_xgb <- XGBTr(feat_train, label_train, par_best_xgb))
save(fit_train_xgb, file="../output/fit_train_xgb.RData")
cat("Time for constructing training features=", tm_feature_train[1], "s \n")
cat("Time for training model=", tm_train_xgb[1], "s \n")
```
```{r}
# super resolution for test (baseline by xgboost package)
# input for debug
